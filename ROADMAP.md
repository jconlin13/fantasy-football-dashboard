...
1. **Splash / home page** — draft date, countdown, draft order with who's paid, a League
   Home button, a Pay Dues button. Modeled on the top of
...
**On the gate.** The site is static, so the check runs in the visitor's browser and is a
speed bump, not security. `draft.ini` holds only a SHA-256, generated by
`pipeline/hash_passphrase.py` from a hidden prompt; blank means no gate. Encrypting the ESPN
link was considered and dropped — it's built from the league id, which is public in
`league.ini`, so there is nothing to hide. **ESPN's own login is the real boundary.**

**Updated 2026-09-03:** the button is now labeled **League Home**, not Launch Draft, and
points at `/football/league?leagueId=...` rather than `/football/draft?...`. Deliberately no
`teamId` in that URL — a `/football/team?...&teamId=1` link would send every visitor to
Jack's specific team, logged in or not, since a teamId addresses one team's data regardless
of who's viewing it. The plain league URL is the one that resolves to whoever is actually
signed in; confirmed by hand, not just inferred, since neither this session's tools nor an
unauthenticated fetch can prove ESPN's personalization behavior on its own.
